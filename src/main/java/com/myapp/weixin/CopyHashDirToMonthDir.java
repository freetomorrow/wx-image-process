...
public class CopyHashDirToMonthDir {
    ...
    public static void main(String[] args) {
        ...
        String date = "/2022-09";
        String imageDate = "/Image" + date;
        String videoDate = "/Video" + date;

        String targetPath = "D:/wx/WeChat Files/***/FileStorage/" + imageDate;
        String targetVideoPath = "D:/wx/WeChat Files/***/FileStorage/" + videoDate;
        if (args != null) {
            if (args.length > 0) {
            }
        }
        ...
        for (File f : file.listFiles()) {
            ...
            convert(f.getAbsolutePath() + imageDate, targetPath);
            //
            copyVideo(f.getAbsolutePath() + videoDate, targetVideoPath);
        }

        CleanEmpty.clean(file);
    }
     ...
     */
    private static void copyVideo(String path, String targetPath) {
        ...
        Arrays.stream(file).parallel().forEach(file1 -> {
            if (file1.isDirectory()) {
                ...
                copyVideo(file1.getPath(), targetPath + File.separator + newTargetPath[newTargetPath.length - 1]);
                return;
            }
            ...
            try {
                ...
                String targetFileName = targetPath + File.separator + file1.getName();
                ...
                System.out.println(file1.getName() + "(大小:" + ((double) file1.length() / 1000) + "kb进度：" + integer.get() +
                        "/" + size);
                targetFile = new File(targetFileName);
                targetFile.setLastModified(file1.lastModified());
                reader.close();
                file1.delete();
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private static void convert(String path, String targetPath) {
        ...
        Arrays.stream(file).parallel().forEach(file1 -> {
            ...
            try {
                ...
                InputStream reader = new FileInputStream(file1);
                String targetFileName = targetPath + File.separator + file1.getName().split("\\.")[0] + (xor[0] != null ?
                        "." + xor[0] : "");
                File targetFile = new File(targetFileName);
                if (targetFile.exists()) {
                    reader.close();
